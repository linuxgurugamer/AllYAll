...
namespace AllYAll
{
    ...
    public class AYA_ModuleAnimateGeneric : ModuleAnimateGeneric
    {
        ...
        public void DoAllParts()                                                                //This runs every time you click "extend all" or "retract all"
        {
            ...
            var callingPart = this.part.FindModuleImplementing<AYA_ModuleAnimateGeneric>();   //Variable for the part doing the work.
            ...
            {
                var thisPartAnimate = eachPart.FindModuleImplementing<AYA_ModuleAnimateGeneric>();   //If it's an antenna...
                if (thisPartAnimate != null && thisPartAnimate.animationName != "")                           //..and it has an animation (rules out ox-stats and the like)
                {
        ...
        {
            if (deployState == ModuleDeployablePart.DeployState.EXTENDING)
                deployState = ModuleDeployablePart.DeployState.EXTENDED;
            if (deployState == ModuleDeployablePart.DeployState.RETRACTING)
